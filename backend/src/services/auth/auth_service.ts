...
export const authService = {
  ...
  async sendOtp(email: string) {
    ...
    // ?? Safe Upsert (Deletes old OTP first implicitly via upsert on primary key if configured, or explicit delete)
  ...
  async verifyOtp(email: string, otp: string) {
    const { data } = await supabase
      ...
      .select('*')
      .eq('email', email)
      .eq('code', otp) // Check code in DB query for speed
      .maybeSingle();
  ...
  verifyPass(email: string, passType: string) {
    ...
    if (passType === 'single' || passType === 'pro') {
      if (!domain?.endsWith('srmist.edu.in')) {
        throw new Error('This pass is exclusively for SRM Verified Students (@srmist.edu.in).');
      }
    }
